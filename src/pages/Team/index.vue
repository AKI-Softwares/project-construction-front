  ...
  <MainLayout titulo="Equipe">
        ...
        <div v-for="role in roles" :key="role.id" class="role-card">
          <div class="role-info">
            ...
            <div class="role-perms">
              ...
              <span v-if="role.permissions.length > 6" class="perm-tag more">
          ...
          <div class="role-meta">
            ...
            <button
              v-if="!role.isSystem && role._count.users === 0"
              class="btn-delete"
    ...
    <div v-if="userToDelete" class="modal-overlay" @click.self="userToDelete = null">
      <div class="modal">
        ...
        <div class="modal-actions">
          <button class="btn-confirm-delete" :disabled="deleting" @click="doDeleteUser">
    ...
    <div v-if="roleToDelete" class="modal-overlay" @click.self="roleToDelete = null">
      <div class="modal">
        ...
        <div class="modal-actions">
          <button class="btn-confirm-delete" :disabled="deletingRole" @click="doDeleteRole">
...
function toggleGroup(group) {
  if (isGroupChecked(group)) {
    roleForm.permissionIds = roleForm.permissionIds.filter(
      id => !group.permissions.map(p => p.id).includes(id)
    )
  } else {
...
function validateRoleForm() {
  ...
  let valid = true
  if (!roleForm.name || roleForm.name.length < 2) {
    roleFormErrors.name = 'Nome deve ter pelo menos 2 caracteres.'
    valid = false
  }
  if (roleForm.permissionIds.length === 0) {
    roleFormErrors.permissionIds = 'Selecione ao menos uma permissão.'
    valid = false
  }
  return valid
}
...
async function submitRole() {
  ...
  try {
    ...
    if (e.response?.status === 409) {
      roleFormErrors.name = 'Já existe uma função com esse nome.'
    } else {
      roleFormError.value = e.response?.data?.message || 'Erro ao criar função.'
    }
  } finally {
...
.tab-btn {
  ...
  background: #0d0d2b; color: #fff; font-size: 0.9rem; font-weight: 600; cursor: pointer;
...
.btn-primary {
  ...
  background: #0d0d2b; color: #fff; border: none; border-radius: 30px;
...
.user-card {
  ...
  background: #6b6b6b; border-radius: 12px; padding: 20px 24px;
...
.form-card {
  ...
  border: 1px solid #eee; max-width: 860px;
...
input[type="text"] {
  ...
  background: #e8e8e8; font-size: 0.95rem; outline: none; color: #333;
}
...
.permissions-grid {
  ...
  gap: 16px; padding: 16px; background: #f8f8f8; border-radius: 12px;
}
...
.btn-save {
  padding: 12px 32px; background: #00e5cc; border: none;
  border-radius: 30px; font-size: 0.95rem; font-weight: bold;
  color: #0d0d2b; cursor: pointer;
}
.btn-save:disabled { opacity: 0.6; cursor: not-allowed; }
.btn-cancel {
  padding: 12px 32px; background: #e8e8e8; border: none;
  border-radius: 30px; font-size: 0.95rem; font-weight: bold;
  color: #333; cursor: pointer;
}
.btn-delete {
  background: none; border: none; color: #ccc; cursor: pointer;
  font-size: 1rem; padding: 6px 8px; border-radius: 6px; transition: color 0.2s;
}
...
.btn-confirm-delete {
  ...
  border-radius: 30px; color: #fff; font-size: 0.95rem; font-weight: bold; cursor: pointer;
}
