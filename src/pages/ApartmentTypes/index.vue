...
// IMPORTAÇÃO CHAVE: Puxa o catálogo global de serviços reais cadastrados
import { getServices } from '../../services/services.js'

const types = ref([])
const catalogServices = ref([]) // Guarda os serviços reais vindos da sua API
...
// 🎯 LÓGICA DO DICIONÁRIO AUTOMÁTICO:
// Mapeia dinamicamente palavras-chave do nome do cômodo para filtrar as categorias do seu banco
function obtenerServicosSugeridos(roomName) {
  if (!roomName || !Array.isArray(catalogServices.value)) return []
  
  const nameLower = roomName.toLowerCase()

  // Se o cômodo contiver "banheiro" ou "woc" ou "suíte"
  if (nameLower.includes('banheiro') || nameLower.includes('wc') || nameLower.includes('bwc')) {
  ...
  // Se contiver "cozinha" ou "área de serviço" ou "lavanderia"
  if (nameLower.includes('cozinha') || nameLower.includes('serviço') || nameLower.includes('lavanderia')) {
    return catalogServices.value.filter(s => {
      const cat = String(s.category || '').toLowerCase()
  ...
  // Se for "quarto" ou "sala" ou "circulação" ou "corredor"
  if (nameLower.includes('quarto') || nameLower.includes('sala') || nameLower.includes('circulação') || nameLower.includes('corredor') || nameLower.includes('suíte')) {
    return catalogServices.value.filter(s => {
      const cat = String(s.category || '').toLowerCase()
      return cat.includes('pintura') || cat.includes('elétrica') || cat.includes('esquadria') || cat.includes('marcenaria')
    })
  }

  // Fallback geral: se não casar com nenhuma palavra, traz serviços de Pintura e Acabamento por padrão
  return catalogServices.value.filter(s => {
    const cat = String(s.category || '').toLowerCase()
...
onMounted(async () => {
  try {
    // Busca em paralelo os Tipos cadastrados e os Serviços Reais da sua outra tela!
