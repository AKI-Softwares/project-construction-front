        ...
        <div v-else class="visit-list">
          <div v-for="visit in recentVisits" :key="visit.id" class="visit-item" @click="openVisit(visit.id)">
            <div class="visit-info">
              ...
              <span class="visit-sub">{{ visit.apartment?.building?.name || '—' }}</span>
            </div>
...
function traduzirStatus(status) {
  ...
  if (s === 'IN_PROGRESS' || s === 'ONGOING') return 'Em andamento'
  return status
}
...
.metric-card { display: flex; flex-direction: column; justify-content: flex-start; align-items: flex-start; gap: 10px; background: #fff; border: 1px solid #eee; border-radius: 12px; padding: 20px 24px; }
.metric-label { font-size: 0.8rem; color: #555; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; }
.metric-value { font-size: 2.6rem; font-weight: 700; color: #11142d; line-height: 1; }
...
.home-layout-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; }
.full-width-panel { grid-column: span 2; }
.content-panel { background: #fff; border: 1px solid #eee; border-radius: 12px; padding: 24px; display: flex; flex-direction: column; }
...
.status-badge.finalized { background: #e0f2f1; color: #004d40; }
.status-badge.ongoing { background: #e8eaf6; color: #1a237e; }
